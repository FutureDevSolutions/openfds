    ...
    Effect.gen(function* () {
      ...
      const connectTransport = (transport: Transport, timeout: number) =>
        Effect.acquireUseRelease(
          ...
          (t) =>
            Effect.tryPromise({
              try: () => {
                const client = new Client({ name: "openfds", version: Installation.VERSION })
              ...
              if (isAuthError) {
                ...
                if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                  ...
                  return bus
                    .publish(TuiEvent.ToastShow, {
                      title: "MCP Authentication Required",
                      message: `Server "${key}" requires authentication. Run: openfds mcp auth ${key}`,
                      variant: "warning",
                      duration: 8000,
                    })
                    .pipe(Effect.ignore, Effect.as(undefined))
      ...
      const connectLocal = Effect.fn("MCP.connectLocal")(function* (key: string, mcp: Config.Mcp & { type: "local" }) {
        ...
        const transport = new StdioClientTransport({
          ...
          env: {
            ...process.env,
            ...(cmd === "openfds" || cmd === "opencode" ? { BUN_BE_BUN: "1" } : {}),
            ...mcp.environment,
          },
        })
