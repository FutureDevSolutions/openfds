...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openfds" || path.basename(source_dir) === ".opencode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openfds", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openfds", "tui.json"),
  }
}
